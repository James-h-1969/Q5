...
def get_new_cfg():
    ...
    for rule in cfg["rules"]:
        ...
        if (len(right_side) == 1 and left_side == "S" and right_side == "a"):
            if right_side == ():
                pass
            else:
            ...
            if left_side == right_side[0] or left_side == right_side[1]:
                ...
                if left_side == right_side[0] and left_side == right_side[1]: #case where they are all the same
                    if checkDeriveToTerminalA(right_side[0], final_variable): #derives to an A
                        ...
                        single_rules.append((left_side + "1", tuple()))
    ...
    rules = sorted(list(set(no_a + one_a + two_a + three_a + four_a + single_rules + [(var[0], (var[1],)) for var in final_variable if var[0] != "S"])), key=lambda x: x[0][0])
    cfg["rules"] = rules
    cfg["V"] = new_Variables
    cfg["start"] = "S0"
